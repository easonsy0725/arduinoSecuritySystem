...
// === CONFIGURATION ===
const ARDUINO_PORT = '/dev/tty.usbmodem101'; // change if needed
...
parser.on('data', (data) => {
  ...
  if (data === 'PANIC') {
    console.log('⚠️ PANIC signal received from Arduino!');
    latestStatus.panic = true;
    return;
  }
...
app.post('/api/clear-panic', (req, res) => {
  latestStatus.panic = false;
  res.json({ success: true });
});
